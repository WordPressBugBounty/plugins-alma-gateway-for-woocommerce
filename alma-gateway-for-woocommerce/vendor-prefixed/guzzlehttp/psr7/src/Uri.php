...
use Alma\Vendor\GuzzleHttp\Psr7\Exception\MalformedUriException;
...
class Uri implements UriInterface, \JsonSerializable
{
    ...
    private static function parse(string $url)
    {
        ...
        $prefix = '';
        $ipv6Prefix = preg_match('%\Alma_Gateway_A([0-9A-Za-z+.-]+://\[[^\]\x00-\x20\x7F/?#@]+\])(.*)\z%s', $url, $matches);
        ...
        if ($ipv6Prefix === 1) {
            ...
            // different host.
            if (preg_match('%\Alma_Gateway_A(?::[0-9]*)?(?:[/?#].*)?\z%s', $suffix) !== 1) {
                return false;
            }
