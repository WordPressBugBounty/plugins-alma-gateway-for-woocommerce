...
use Alma\Vendor\GuzzleHttp\Exception\BadResponseException;
use Alma\Vendor\GuzzleHttp\Exception\RequestException;
use Alma\Vendor\GuzzleHttp\Exception\TooManyRedirectsException;
...
class RedirectMiddleware
{
    ...
    public function checkRedirect(RequestInterface $request, array $options, ResponseInterface $response)
    {
        ...
        if (\Alma\Vendor\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $nextRequest->getUri()) && defined('\Alma_Gateway_CURLOPT_HTTPAUTH')) {
            unset($options['curl'][\Alma_Gateway_CURLOPT_HTTPAUTH], $options['curl'][\Alma_Gateway_CURLOPT_USERPWD]);
        }
    ...
    public function modifyRequest(RequestInterface $request, array $options, ResponseInterface $response): RequestInterface
    {
        ...
        if (!isset($modify['body'])) {
            try {
                \Alma\Vendor\GuzzleHttp\Psr7\Message::rewindBody($request);
        ...
        if (\Alma\Vendor\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $modify['uri'])) {
            $modify['remove_headers'][] = 'Authorization';
            $modify['remove_headers'][] = 'Cookie';
        }
        return \Alma\Vendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
    }
    /**
    ...
    {
        $location = \Alma\Vendor\GuzzleHttp\Psr7\UriResolver::resolve($request->getUri(), new Psr7\Uri($response->getHeaderLine('Location')));
