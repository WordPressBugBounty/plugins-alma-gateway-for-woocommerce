...
use Alma\Vendor\GuzzleHttp\Cookie\CookieJarInterface;
...
use Alma\Vendor\GuzzleHttp\Handler\CurlVersion;
 ...
 * @final
 */
class Client implements ClientInterface, \Alma\Vendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \Alma\Vendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    {
        alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
        if (\count($args) < 1) {
        ...
        $method = \Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
    ...
    {
        $normalizedMethod = \Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
        ...
        if ($droppedHeaderNames !== [] && isset($options['_conditional'])) {
            $options['_conditional'] = \Alma\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove($droppedHeaderNames, $options['_conditional']);
        }
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = \Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = true;
    ...
    private function configureDefaults(array $config): void
    {
        ...
        if (!isset($this->config['headers'])) {
            ...
            foreach (\array_keys($this->config['headers']) as $name) {
                if (\Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower((string) $name) === 'user-agent') {
                    $hasUserAgent = true;
                    break;
                }
            }
    ...
    private static function warnAboutRequestLevelHandler(array $options): void
    {
        ...
        alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        alma_gateway_trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \Alma\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            $value = $options['auth'];
            $type = isset($value[2]) ? \Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower($value[2]) : 'basic';
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \Alma\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
                    $modify['set_headers']['Authorization'] = 'Basic ' . \base64_encode("{$value[0]}:{$value[1]}");
                    break;
                case 'digest':
                    // @todo: Do not rely on curl
                    $options['curl'][\Alma_Gateway_CURLOPT_HTTPAUTH] = \Alma_Gateway_CURLAUTH_DIGEST;
                    $options['curl'][\Alma_Gateway_CURLOPT_USERPWD] = "{$value[0]}:{$value[1]}";
                    break;
                case 'ntlm':
                    alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. NTLM is also deprecated by curl/libcurl and may be unavailable in current or future libcurl builds. Avoid NTLM; if you must use it temporarily, configure cURL HTTP authentication options directly with a libcurl build that still supports NTLM.');
                    ...
                    $options['curl'][\Alma_Gateway_CURLOPT_HTTPAUTH] = \Alma_Gateway_CURLAUTH_NTLM;
                    $options['curl'][\Alma_Gateway_CURLOPT_USERPWD] = "{$value[0]}:{$value[1]}";
                    break;
            }
        }
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        if (\is_resource($body) || $body === null || \is_string($body) || $body instanceof \Iterator) {
            return \Alma\Vendor\GuzzleHttp\Psr7\Utils::streamFor($body);
        }
        if (\is_scalar($body)) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
            return \Alma\Vendor\GuzzleHttp\Psr7\Utils::streamFor(self::stringifyScalar($body));
        }
        if (\is_object($body) && \method_exists($body, '__toString')) {
            return \Alma\Vendor\GuzzleHttp\Psr7\Utils::streamFor((string) $body);
        }
        if (\is_callable($body)) {
            return \Alma\Vendor\GuzzleHttp\Psr7\Utils::streamFor($body);
        }
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option): array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
