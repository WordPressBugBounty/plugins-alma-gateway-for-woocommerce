...
class Client implements ClientInterface, \Alma\Vendor\Psr\Http\Client\ClientInterface
{
    ...
    private static function warnAboutInvalidRequestOptionTypes(array $options): void
    {
        ...
        if (isset($options['delay']) && (\is_int($options['delay']) || \is_float($options['delay'])) && (!\is_finite((float) $options['delay']) || $options['delay'] < 0)) {
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. Configure NTLM with cURL HTTP authentication options instead.');
