    ...
    public function __invoke(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (is_string($uri) && $type = \Alma\Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($uri)) {
        ...
        return $fn(\Alma\Vendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
