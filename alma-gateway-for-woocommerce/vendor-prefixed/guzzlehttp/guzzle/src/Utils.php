...
use Alma\Vendor\GuzzleHttp\Handler\CurlHandler;
use Alma\Vendor\GuzzleHttp\Handler\CurlMultiHandler;
use Alma\Vendor\GuzzleHttp\Handler\CurlShareHandleState;
use Alma\Vendor\GuzzleHttp\Handler\CurlVersion;
use Alma\Vendor\GuzzleHttp\Handler\Proxy;
use Alma\Vendor\GuzzleHttp\Handler\StreamHandler;
...
final class Utils
{
    ...
    {
        alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.12', '%s() is deprecated and will be removed in 8.0. Use get_debug_type() instead.', __METHOD__);
    ...
    public static function debugResource($value = null)
    {
        ...
        return \Alma\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://output', 'w');
    }
    /**
    ...
    public static function normalizeHeaderKeys(array $headers): array
    {
        ...
        foreach (\array_keys($headers) as $key) {
            $result[\Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower((string) $key)] = $key;
        }
        return $result;
    }
    /**
    ...
    private static function parseNoProxyHost(string $host, ?int $port, bool $matchesRoot): ?array
    {
        if ($host !== '' && $host[0] === '[') {
            ...
            if (!\filter_var($address, \Alma_Gateway_FILTER_VALIDATE_IP, \Alma_Gateway_FILTER_FLAG_IPV6)) {
                return null;
            }
            $host = $address;
        }
        ...
        return ['type' => 'domain', 'value' => \Alma\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower($host), 'port' => $port, 'matchesRoot' => $matchesRoot];
    }
    /**
    ...
    {
        if (!\filter_var($ip, \Alma_Gateway_FILTER_VALIDATE_IP)) {
            return false;
        }
        return \inet_pton($ip);
    }
    ...
    public static function normalizeIdnConversionOption($value): ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', \get_debug_type($value));
            return (int) $value;
        }
    ...
    private static function idnToAsci(string $domain, int $options, ?array &$info = [])
    {
        ...
        throw new \Alma_Gateway_Error('ext-idn or symfony/polyfill-intl-idn not loaded or too old');
    }
}