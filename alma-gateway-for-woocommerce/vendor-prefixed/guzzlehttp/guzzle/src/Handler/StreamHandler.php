...
class StreamHandler
{
    ...
    private function resolveHost(RequestInterface $request, array $options): UriInterface
    {
        ...
        if (isset($options['force_ip_resolve']) && !\filter_var($uri->getHost(), \Alma_Gateway_FILTER_VALIDATE_IP)) {
            if ('v4' === $options['force_ip_resolve']) {
                $records = \dns_get_record($uri->getHost(), \Alma_Gateway_DNS_A);
            ...
            if ('v6' === $options['force_ip_resolve']) {
                $records = \dns_get_record($uri->getHost(), \Alma_Gateway_DNS_AAAA);
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (
            ...
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl_share" option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support cURL sharing.');
        }
        ...
        if (
            ...
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }

        if (self::usesDigestAuth($options)) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing digest authentication to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject digest authentication with the stream handler because it is only supported by cURL handlers.');
        }

        if (\array_key_exists('expect', $options) && $options['expect'] !== false && $request->hasHeader('Expect')) {
            alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    ...
    private static function isCurlOptionGeneratedByAuth(array $options): bool
    {
        ...
        return $httpAuth !== null
            ...
            && isset($options['curl'][\Alma_Gateway_CURLOPT_HTTPAUTH], $options['curl'][\Alma_Gateway_CURLOPT_USERPWD])
            && $options['curl'][\Alma_Gateway_CURLOPT_HTTPAUTH] === $httpAuth;
    }
