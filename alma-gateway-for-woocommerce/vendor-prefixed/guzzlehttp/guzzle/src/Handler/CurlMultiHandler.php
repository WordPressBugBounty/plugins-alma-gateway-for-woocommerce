...
class CurlMultiHandler
{
    ...
    /** @var resource|\Alma_Gateway_CurlMultiHandle */
    private $_mh;
    ...
    public function __construct(array $options = [])
    {
        foreach ($options as $name => $_) {
            if (!isset(self::KNOWN_CONSTRUCTOR_OPTIONS[$name])) {
                alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('The "%s" CurlMultiHandler constructor option is unknown; guzzlehttp/guzzle 8.0 will reject unknown constructor options.', (string) $name));
            }
        }
        ...
        if (isset($options['select_timeout'])) {
            ...
            if (!\is_int($selectTimeout) && !\is_float($selectTimeout) && (!\is_string($selectTimeout) || !\is_numeric($selectTimeout))) {
                alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.14', 'Passing a non-numeric "select_timeout" CurlMultiHandler option is deprecated; guzzlehttp/guzzle 8.0 will reject it.');
            } else {
                $seconds = (float) $selectTimeout;
                if (!\is_finite($seconds) || $seconds < 0 || ($seconds > 0 && (int) ($seconds * 1000) === 0)) {
                    alma_gateway_trigger_deprecation('guzzlehttp/guzzle', '7.14', 'Passing a "select_timeout" CurlMultiHandler option that is not 0 or greater than or equal to 0.001 seconds is deprecated; guzzlehttp/guzzle 8.0 will reject it.');
                }
            }
        ...
        if (\is_array($multiOptions)) {
            ...
            if ($this->multiplexDisabled && \array_key_exists(\Alma_Gateway_CURLMOPT_PIPELINING, $multiOptions)) {
        ...
        if (\is_array($multiOptions)) {
            ...
            if ($this->multiplexDisabled) {
                ...
                $this->options[\Alma_Gateway_CURLMOPT_PIPELINING] = 0;
                $this->requiredOptions[\Alma_Gateway_CURLMOPT_PIPELINING] = true;
            }
        }
     ...
     * @param string $name
     *
     * @return resource|\Alma_Gateway_CurlMultiHandle
     *
     * @throws \Alma_Gateway_BadMethodCallException   when another field as `_mh` will be gotten
    ...
    {
        if ($name !== '_mh') {
            throw new \Alma_Gateway_BadMethodCallException("Can not get other property as '_mh'.");
        }
        ...
        try {
            foreach ($this->options as $option => $value) {
                ...
                \trigger_error(\sprintf('Unable to apply the cURL multi option %s; it was ignored by the runtime libcurl.', self::formatCurlMultiOption($option)), \Alma_Gateway_E_USER_WARNING);
            }
        } catch (\Throwable $e) {
    ...
    private function rejectMultiplexPipeliningConflict(EasyHandle $easy, array $options): void
    {
        ...
        if (!\is_array($this->options) || !\array_key_exists(\Alma_Gateway_CURLMOPT_PIPELINING, $this->options)) {
            // A legacy non-array "options" value is tolerated by the
            // constructor and cannot contain the option.
            return;
        }

        $pipelining = $this->options[\Alma_Gateway_CURLMOPT_PIPELINING];
        ...
        $multiplexBit = \defined('CURLPIPE_MULTIPLEX') ? \Alma_Gateway_CURLPIPE_MULTIPLEX : 2;
    ...
    private function applyMultiplexNone(EasyHandle $easy, array $options): void
    {
        ...
        if (\defined('CURLMOPT_PIPELINING') && \is_array($this->options) && \array_key_exists(\Alma_Gateway_CURLMOPT_PIPELINING, $this->options)) {
        ...
        if (true !== curl_setopt($easy->handle, \Alma_Gateway_CURLOPT_FRESH_CONNECT, true)) {
    ...
    private static function triggerConflictingCurlMultiOptionDeprecations(array $options): void
    {
        ...
        foreach ($options as $option => $_) {
            if (\array_key_exists($option, $conflictingOptions)) {
                alma_gateway_trigger_deprecation('guzzlehttp/guzzle', $sinceOverrides[$option] ?? '7.14', \sprintf('Passing %s in the cURL multi handler "options" is deprecated; guzzlehttp/guzzle 8.0 will reject this option. Use %s instead.', self::formatCurlMultiOption($option), $conflictingOptions[$option]));
            }
        }
    }
    ...
    private static function conflictingCurlMultiOptionSinceOverrides(): array
    {
        ...
        return [\Alma_Gateway_CURLMOPT_PIPELINING => '7.15'];
    }
    ...
    private function addCurlHandle(EasyHandle $easy): void
    {
        ...
        if (\Alma_Gateway_CURLM_OK !== $result) {
