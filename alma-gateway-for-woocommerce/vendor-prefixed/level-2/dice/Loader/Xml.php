 ...
 * @version 3.0 */
namespace Alma\Vendor\Dice\Loader;
class Xml {
	private function getComponent(\SimpleXmlElement $element, $forceInstance = false) {
		if ($forceInstance) return [\Alma_Gateway_Dice\Dice::INSTANCE => (string) $element];
		else if ($element->instance) return [\Alma_Gateway_Dice\Dice::INSTANCE => (string) $element->instance];
		else return (string) $element;
	}

	private function loadV1(\SimpleXmlElement $xml, \Alma_Gateway_Dice\Dice $dice) {
		$rules = [];
	...
	private function loadV2(\SimpleXmlElement $xml, \Alma_Gateway_Dice\Dice $dice) {
		$rules = [];
	...
	public function load($xml, \Alma_Gateway_Dice\Dice $dice = null, $displayWarning = true) {
		...
		if ($dice === null) $dice = new \Alma_Gateway_Dice\Dice;
